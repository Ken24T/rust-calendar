...
- Linux build dependency bootstrap helper for Debian/Ubuntu/Mint development and SHIP verification setup.

### Changed

- README and contributing guidance now direct Linux developers to `./scripts/install-linux-build-deps.sh`, including a dry-run preview mode for the apt install plan.

## [2.4.35] - 2026-03-26

### Added

- Repository TCTBP runtime support files for explicit trigger routing, safety hooks, and installation/export of the workflow into other repositories.

### Changed

- Reconciled the Rust Calendar TCTBP workflow documentation and machine-readable profile around the guarded `publish`, `handover`, `resume`, `status`, `abort`, and `branch` flows.
- Refreshed Copilot repo instructions so the runtime, workflow profile, and operator guidance stay aligned.
...
- Expanded monthly `BYDAY` RRULE support to correctly handle mixed and multi-token
  forms, including both positional entries (for example `1FR`, `-1SU`) and
  non-positional weekday lists (for example `MO,WE`) within monthly recurrence
  generation.
- Improved monthly recurrence date candidate generation by deduplicating and
  sorting expanded `BYDAY` matches, ensuring deterministic output across mixed
  rule combinations.
...
- Consolidated all import/export options into the File menu for discoverability
  (Import Events, Import Countdown Layout, Export Events, Export to PDF, Export
  Countdown Layout).
- Simplified Events menu to New Event, Templates, and Search only.
- Renamed "Manage Templates" to "Manage Event Templates" for clarity.
