...
mod tests {
    ...
    fn test_new_event_invalid_times() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Event end time must be after start time");
    }
    ...
    fn test_builder_with_optional_fields() {
        ...
        assert_eq!(event.description, Some("Annual tech conference".to_string()));
    ...
    fn test_recurrence_rule() {
        ...
        assert_eq!(event.recurrence_rule, Some("FREQ=WEEKLY;BYDAY=MO".to_string()));
        assert!(event.is_recurring());
    }
}
