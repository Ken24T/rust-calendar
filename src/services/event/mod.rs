...
impl<'a> EventService<'a> {
    ...
    pub fn create(&self, mut event: Event) -> Result<Event> {
        ...
        let exceptions_json = event.recurrence_exceptions.as_ref().map(|excs| {
            let dates: Vec<String> = excs.iter().map(|dt| dt.to_rfc3339()).collect();
        ...
        let result = self.conn.query_row(
            ...
            |row| {
                ...
                Ok(Event {
                    ...
                    created_at: Some(
                        DateTime::parse_from_rfc3339(&row.get::<_, String>(11)?)
                            .map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?
                            .with_timezone(&Local),
                    ),
                    updated_at: Some(
                        DateTime::parse_from_rfc3339(&row.get::<_, String>(12)?)
                            .map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?
                            .with_timezone(&Local),
                    ),
                })
    ...
    pub fn update(&self, event: &Event) -> Result<()> {
        let id = event
            .id
            .ok_or_else(|| anyhow::anyhow!("Event ID is required for update"))?;
                ...
                if !byday_days.is_empty() {
                    ...
                    let mut current_week_start = event.start.date_naive()
                        - chrono::Duration::days(
                            event.start.weekday().num_days_from_monday() as i64
                        );
                    ...
                    loop {
                        ...
                        for &target_weekday in &byday_days {
                            ...
                            let occurrence_date =
                                current_week_start + chrono::Duration::days(days_offset);
                                ...
                                .single()
                            {
                                ...
                                if occurrence_datetime >= event.start {
                                    ...
                                    if !is_exception {
                                        ...
                                        if occurrence_datetime >= range_start
                                            && occurrence_datetime <= range_end
                                        {
                        ...
                        current_week_start =
                            current_week_start + chrono::Duration::weeks(interval as i64);
                ...
                if rrule.contains("FREQ=MONTHLY") {
                        ...
                        let occurrence_date = if let Some(day) = bymonthday {
                            ...
                            if position == 1 {
                                ...
                                next_month
                                    ...
                                    .and_then(|last_of_month| {
                                        let last_weekday = last_of_month.weekday();
                                        let days_back_to_target =
                                            ((last_weekday.num_days_from_monday() as i32
                                                - weekday.num_days_from_monday() as i32
                                                + 7)
                                                % 7)
                                                as i64;
                                        Some(
                                            last_of_month
                                                - chrono::Duration::days(days_back_to_target),
                                        )
                                    })
                            }
                        } else {
                    ...
                    loop {
                        ...
                        let is_exception = if let Some(ref exceptions) = event.recurrence_exceptions
                        {
                            exceptions
                                .iter()
                                .any(|ex| ex.date_naive() == current_start.date_naive())
                        } else {
                            false
                        };
                        ...
                        current_start = current_start.with_year(new_year).unwrap_or(
                            current_start + chrono::Duration::days(365 * interval as i64),
                        );
